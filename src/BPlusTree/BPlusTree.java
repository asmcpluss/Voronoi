...
//		//not using bloom filter about this project
...
//			continueToSearch = filter.contains(key); // find the given key in the filter
		...
		if (continueToSearch){// if the bloom filter return true (it might happen when the key really exists, or either false positive)
			LeafNode<Value> node = findLeaf(key);
			if(node.getValue(key)==null){
				List<Integer> keys = node.keys;
				if(key<keys.get(0)){//should search the previous leaf nodes
					while(node.getPrev()!=null){
						if(key>node.getPrev().keys.get(node.getPrev().keys.size()-1)){
							node = node.getPrev();
							break;
						}
						node = node.getPrev();
				...
				else{ //should search the next leaf nodes
					while(node.getNext()!=null){
						if(key<node.getNext().keys.get(0)){
							break;
						}
						node = node.getNext();
					}
				}
				if(node.getValue(key)!=null){ //遍历后的结点
					...
					if(keys.get(keys.size()-1)<key){
						return node.getValue(keys.get(keys.size()-1));
					}
					else{
						for(;i<keys.size();i++){
							if(keys.get(i)>key){
						...
						return node.getValue(keys.get(i-1));
					}
				}
			}
			else{
