...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    ***************************************************************************/
     ...
     *
     * @param  xcoord an array of all the <em>x</em>-coordinates of the polygon
     * @param  ycoord an array of all the <em>y</em>-coordinates of the polygon
     * @throws IllegalArgumentException unless {@code x[]} and {@code y[]}
     *         are of the same length
     */
    public static void polygon(double[] xcoord, double[] ycoord) {
        if (xcoord == null) throw new NullPointerException();
        if (ycoord == null) throw new NullPointerException();
        int n1 = xcoord.length;
        int n2 = ycoord.length;
        if (n1 != n2) throw new IllegalArgumentException("arrays must be of the same length");
        int n = n1;
        GeneralPath path = new GeneralPath();
        path.moveTo((float) scaleX(xcoord[0]), (float) scaleY(ycoord[0]));
        for (int i = 0; i < n; i++)
            path.lineTo((float) scaleX(xcoord[i]), (float) scaleY(ycoord[i]));
        path.closePath();
        offscreen.draw(path);
        draw();
    }
